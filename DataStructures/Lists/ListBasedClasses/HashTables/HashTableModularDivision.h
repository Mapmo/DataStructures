...
private:
	...
	doubleLinkedList1<T, keyType> * m_Collisions;
};

template<class T, class keyType>
inline HashTableModularDivision<T, keyType>::HashTableModularDivision(const unsigned int numb) : m_Modul(numb), m_Collisions(new doubleLinkedList1<T, keyType>[numb])
{
}
...
{
	doubleLinkedList1<T, keyType> tmp(val, key);
